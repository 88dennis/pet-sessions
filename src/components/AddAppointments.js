...
const AddAppointments = (props) => {
  ...
  const handleAdd = (e) => {
    ...
    if (!state.aptDate && !state.aptTime) {
      ...
      console.log("*******", state.aptDate + " " + state.aptTime);
    } else if (!state.aptDate && state.aptTime) {
      aptDateToLog =
    ...
    let tempApt = {
      ...
      // aptDate: aptDate ? new Date(aptDate).getTime() : new Date().toDateString() + ' ' + aptTime,
  ...
  return  <div
        ...
        >
          ...
          >
           ...
           {/* {props.formDisplay ? <FaMinus /> : <FaPlus />} Add Appointment */}
          </div>
